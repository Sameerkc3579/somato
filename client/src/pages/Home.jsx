...
  // --- FETCH REAL COUNTS FOR COLLECTIONS (UPDATED) ---
  useEffect(() => {
    // 🚨 CRITICAL: Encode the city for URL use 🚨
    const cityQuery = encodeURIComponent(city); 

    const fetchCounts = async () => {
      try {
        const [trending, events, newPlaces, veggie] = await Promise.all([
          // 🚨 URL UPDATED to include ?city=... 🚨
          fetch(`/api/collections/trending?city=${cityQuery}`).then(res => res.json()),
          fetch(`/api/collections/events?city=${cityQuery}`).then(res => res.json()),
          fetch(`/api/collections/new?city=${cityQuery}`).then(res => res.json()),
          fetch(`/api/collections/veggie?city=${cityQuery}`).then(res => res.json())
        ]);
        
        setCounts({
...
    fetchCounts();
  }, [city]); // 🚨 Dependency added: Fetches again whenever the city changes!
