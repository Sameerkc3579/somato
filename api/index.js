...
app.get("/api/collections/:type", async (req, res) => {
  try {
    ...
    // 🚨 GET CITY FROM QUERY STRING (e.g., ?city=Patna)
    const { city } = req.query; 
    
    // START QUERY: Always filter by city first
    let query = city ? { city: city } : {}; 

    if (type === "trending") {
      // Trending: Rating 4.0+ AND city filter
      query = { ...query, rating: { $gte: 4.0 } };
    } else if (type === "veggie") {
      // Veggie: isVeg=true AND city filter
      query = { ...query, isVeg: true };
    } else if (type === "new") {
      // New: Sort by newest AND city filter
      const results = await Restaurant.find(query).sort({ _id: -1 }).limit(10);
      return res.json(results);
    } else if (type === "events") {
      // Nightlife: City filter AND one of the $or conditions
      query = { 
        ...query, // <- This ensures the city filter is maintained
        $or: [
